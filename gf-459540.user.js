...
v5.2 hls.js库去广告效率提升30%；修正手机页面的不适配，包括PC下访问手机页面的情形
...
(async () => {
	...
	const isWeb = !location.host.startsWith('m.');
	...
	//豆瓣影片名及其年份
	let vName = isWeb ? document.title.slice(0, -5) : $(".sub-title").innerText;
	let videoYear = isWeb ? $(".year").innerText.slice(1, -1) :
		$(".sub-original-title").innerText.slice(-5, -1);
	...
	//播放按钮
	function playBtn() {
		const e = html2DOM(`<xy-button type="primary">播放</xy-button><xy-button type="primary">⚙片名年份</xy-button>`,
			$(isWeb ? "h1" : ".sub-original-title"));
	...
	class UI {
		constructor(playList) {
			...
			const q = e.querySelector.bind(e);
			q(".cacheSize").onclick = function() {
				const n = +prompt('请输入视频缓存区大小，区间：15 － 800整数秒',''+ buffSize);
				if (n > 14 && n < 801) GM_setValue('buffSize', n|0);
			};
			q(".pot-playList").onclick = async function(ev){
				ev.stopPropagation();
			...
			e.children[0].onclick = function() {
				...
				if (!isWeb) $('#TalionNav').hidden = $('.talion-nav-footer').hidden = !1;
			};
			document.body.style.overflow = 'hidden';
			if (!isWeb) $('#TalionNav').hidden = $('.talion-nav-footer').hidden = true;
			q(".next-series").onclick = function() {
				q('.play + xy-button')?.click();
			};
